...
export const Chat: React.FunctionComponent<{ info: ChatInfo }> = ({ info: ChatInfo }) => {
    ...
    const keyUpSubmitHandler = (event: React.KeyboardEvent<HTMLInputElement>) => {
        if (event.key == "Enter") {
            submitHandler()
        }
    }
    ...
    return <div className="chat">
        <div className="wrapper">
            ...
            <div className={`sendChat ${ChatInfo.isPending ? ' Pending' : ''}`}>
                <input type="text" placeholder="傳送訊息" value={message} onChange={(e) => { setMessage(e.target.value) }} onKeyUp={keyUpSubmitHandler} disabled={ChatInfo.isPending} />
